...
if (grid) {
    grid.addEventListener("click", function () {
        viewType = 'grid';
        console.log(document.querySelector(".products-area-wrapper"))
        ...
        document
            .querySelectorAll(".products-area-wrapper").forEach(i => i.classList.remove("tableView"));
...
function getItems(area, controllerName, actionName, pageNumber, searchText, filterValue, sortBy) {
    ...
    $.ajax({
        type: "GET",
        url: url,
        data: { searchText, filterValue, sortBy, pageNumber },
...
function getItemCollectionByParent(area, controllerName, actionName, itemId, searchText, filterValue, sortBy, pageNumber) {
    let url = `/${controllerName}/${actionName}`;
    if (area) {
        url = `/${area}/${controllerName}/${actionName}`;
    }
    $.ajax({
        type: "GET",
        url: url,
        data: {id: itemId, searchText, filterValue, sortBy, pageNumber },
        success: function (response) {
            $("#results-container").html(response);
        },
        failure: function (response) {
...
function getUDpartial(area, controllerName, actionName, id) {
    let url = `/${controllerName}/${actionName}`;
    if (area) {
        url = `/${area}/${controllerName}/${actionName}`;
    }
